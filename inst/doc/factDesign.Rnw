...
%\VignetteDepends{Biobase,annotate,affy,factDesign}
...
<<smalleffect,eval=TRUE,fig=TRUE>>=
...
\noindnet Again, we can use a heatmap to look at genes for which we rejected $H_{ES,TIME*}.$  
